...
reader=csv.DictReader(open("ufo5.csv","rU")) 
...
for row in reader:     
	...
	if not row['Sighting Date'] in dtList:
		print "Creating node ", row['Sighting Date']
		sightID = row['Sighting Date']
		DG.add_node(sightID, typ='sight')
		dtList.append(row['Sighting Date'])
...
#write graph
nx.write_graphml(DG, 'ufo5.graphml')
